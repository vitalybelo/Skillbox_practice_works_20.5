 ...
 * = 200, просто так, из развлечения. Вместо нулей и единичек - пробелы и звездочки.
 * Случайные значения добываются из библиотеки random в диапазоне dist().
 * Зерно и значения не детерминированные, откуда берется не знаю ))
...
int main() {
    ...
    pic_width = min(pic_width, 200);
    pic_height = min(pic_height, 200);
